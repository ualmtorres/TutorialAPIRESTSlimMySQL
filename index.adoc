...
* https://github.com/ualmtorres/slim-base-rest-api.gitπ[repositorio, window=_blank] con proyecto base. Sólo contiene la configuración de Slim y una ruta de ejemplo.
====
...
function createJsonResponse(ResponseInterface $response, array $data): ResponseInterface { <3>
    // Set the response content type
    $response = $response->withHeader('Content-Type', 'application/json; charset=utf-8');
...
$mysqli = new mysqli($host, $username, $password, $database); <4>
...
$app->get('/product', function (RequestInterface $request, ResponseInterface $response, array $args) { <5>
    global $mysqli;
...
$app->get('/product/{id}', function (RequestInterface $request, ResponseInterface $response, array $args) {
    ...
    $stmt = $mysqli->prepare($query); <8>
    $stmt->bind_param('i', $productId); <9>
    $stmt->execute(); <10>
    $result = $stmt->get_result(); <11>

    // Comprobar si hay un producto
    if ($result->num_rows > 0) {
        $product = $result->fetch_assoc(); <12>
...
$app->post('/product', function (RequestInterface $request, ResponseInterface $response, array $args) { <13>
    global $mysqli;

    // Obtener el cuerpo de la solicitud
    $body = $request->getParsedBody(); <14>
    ...
    $query = "INSERT INTO product (name, price, created_at, updated_at) VALUES (?, ?, ?, ?)"; <16>
    $stmt = $mysqli->prepare($query); <17>
    $stmt->bind_param('sdss', $name, $price, $createdAt, $updatedAt); <18>
    $result = $stmt->execute(); <19>
...
$app->put('/product/{id}', function (RequestInterface $request, ResponseInterface $response, array $args) { <20>
    global $mysqli;
    ...
    $stmt = $mysqli->prepare($query); <22>
    $stmt->bind_param('i', $productId); <23>
    $stmt->execute(); <24>
    $result = $stmt->get_result();
    ...
    if ($result->num_rows > 0) {
        ...
        $query = "UPDATE product SET name = COALESCE(?, name), price = COALESCE(?, price), updated_at = ? WHERE id = ?"; <25>
        $stmt = $mysqli->prepare($query);
...
$app->delete('/product/{id}', function (RequestInterface $request, ResponseInterface $response, array $args) { <27>
    global $mysqli;
    ...
    // Comprobar si el producto existe
    $query = "SELECT * FROM product WHERE id = ?"; <28>
    $stmt = $mysqli->prepare($query); <29>
    $stmt->bind_param('i', $productId); <30>
    $stmt->execute(); <31>
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        // Eliminar el producto
        $query = "DELETE FROM product WHERE id = ?"; <32>
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('i', $productId);
        $result = $stmt->execute(); <33>
...
// Interceptar todas las rutas no definidas 
$app->any('{routes:.+}', function (RequestInterface $request, ResponseInterface $response) { <34>
    $data = [
        'status' => 404,
        'message' => 'Route not found'
    ];
...
<2> Configurar Slim para procesar datos JSON.
<3> Función auxiliar para manejar la respuesta de la API.
<4> Establecer la conexión a la base de datos MySQL.
<5> Endpoint para obtener todos los productos.
<6> Acceder al parámetro de la URL `id`.
<7> Consulta para obtener un producto por ID.
<8> Preparar la consulta para obtener un producto por ID.
<9> Enlazar el ID del producto a la consulta.
<10> Ejecutar la consulta.
<11> Obtener el resultado de la consulta.
<12> Obtener el producto por ID.
<13> Endpoint para crear un nuevo producto.
<14> Obtener los datos del cuerpo de la solicitud.
<15> Obtener el nombre y el precio del producto.
<16> Consulta para crear un nuevo producto.
<17> Preparar la consulta para crear un nuevo producto.
<18> Enlazar los valores a la consulta.
<19> Ejecutar la consulta para crear un nuevo producto.
<20> Endpoint para actualizar un producto.
<21> Consulta para comprobar si el producto existe.
<22> Preparar la consulta para comprobar si el producto existe.
<23> Enlazar el ID del producto a la consulta.
<24> Ejecutar la consulta.
<25> Consulta para actualizar un producto.
<26> Ejecutar la consulta para actualizar un producto.
<27> Endpoint para eliminar un producto.
<28> Consulta para comprobar si el producto existe.
<29> Preparar la consulta para comprobar si el producto existe.
<30> Enlazar el ID del producto a la consulta.
<31> Ejecutar la consulta.
<32> Consulta para eliminar un producto.
<33> Ejecutar la consulta para eliminar un producto.
<34> Middleware para interceptar rutas no definidas.
